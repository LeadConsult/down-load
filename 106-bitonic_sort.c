...
/**
 * bitonic_compare - sort  values in a sub-array with respect to
...
void bitonic_compare(char up, int *array, size_t size)
{
	...
	{
		if ((array[index] > array[index + dist]) == up)
		{
			swap = array[index];
			array[index] = array[index + dist];
			array[index + dist] = swap;
		}
	}
}
...
/**
 * bit_sort - recursive function using  Bitonic sort algorithm
 * @up: direction of sorting
 * @array: sub-array to sort
 * @size: size of  sub-array
 * @t: total size of  original array
 *
 * Return: void
 */
