...
/**
 * card_value - returns  value of a card
 * @node: card in a deck
 *
...
int card_value(deck_node_t *node)
{
	...
	int index, kind_val = 0;

	for (index = 1; index <= 13; index++)
	{
		if (!_strcmp(node->card->value, val[index - 1]))
			kind_val = index;
	}

	for (index = 1; index <= 4; index++)
	{
		if (!_strcmp(kinds[node->card->kind], kinds[index - 1]))
			kind_val = kind_val + (13 * index);
	}
