#include "sort.h"
#include <stdlib.h>
#include <stdio.h>

/**
 * TDMerge - sorts and merges  sub arrays in source
 * @start: starting index (inclusive) for  left sub array
 * @middle: end index (exclusive) for  left sub array and
 * starting index (inclusive) for  right sub array
 * @end: end index (exclusive) for  right sub array
 * @dest: destination for data
 * @source: source of data
 *
 * Return: void
 */
void TDMerge(size_t start, size_t middle, size_t end, int *dest, int *source)
{
	size_t x, j, k;

	printf("Merging...\n");
	printf("[left]: ");
	print_array(source + start, middle - start);
	printf("[right]: ");
	print_array(source + middle, end - middle);
	x = start;
	j = middle;
	for (k = start; k < end; k++)
	{
		if (x < middle && (j >= end || source[x] <= source[j]))
		{
			dest[k] = source[x];
			x++;
		}
		else
		{
			dest[k] = source[j];
			j++;
		}
	}
	printf("[Done]: ");
	print_array(dest + start, end - start);
}

/**
 * TDSplitMerge - recursively splits  array and merges  sorted arrays
 * @start: starting index (inclusive)
 * @end: end index (exclusive)
 * @array:  array to sort
 * @copy: a copy of  array
 *
 * Return: void
 */
void TDSplitMerge(size_t start, size_t end, int *array, int *copy)
{
	size_t middle;

	if (end - start < 2)
		return;
	middle = (start + end) / 2;
	TDSplitMerge(start, middle, array, copy);
	TDSplitMerge(middle, end, array, copy);
	TDMerge(start, middle, end, array, copy);
	for (middle = start; middle < end; middle++)
		copy[middle] = array[middle];
}

/**
 * merge_sort - sorts an array of integers in ascending order using
 * Merge sort algorithm
 * @array: array to sort
 * @size: size of  array
 *
 * Return: void
 */
void merge_sort(int *array, size_t size)
{
	size_t x;
	int *copy;

	if (array == NULL || size < 2)
		return;
	copy = malloc(sizeof(int) * size);
	if (copy == NULL)
		return;
	for (x = 0; x < size; x++)
		copy[x] = array[x];
	TDSplitMerge(0, size, array, copy);
	free(copy);
}
